...
const getDirection = deg => {
  ...
  const updateDom = data => {
    ...
    const Tempconverter = (temp) => {
      const fmintemp = (temp - 32) / 1.8
      return fmintemp
    }
  // Current temperature
  
  const maintemp= data[0].main.temp
  currentTemperature.innerText = Tempconverter(maintemp).toFixed(2)

  document.getElementById('yo').onclick =() => {

    if (document.getElementById('yo').innerText =='℃'){

      currentTemperature.innerText = maintemp.toFixed(2)
      document.getElementById('yo').innerText = '°F'
    } 
    else if(document.getElementById('yo').innerText =='°F'){
      currentTemperature.innerText = Tempconverter(maintemp).toFixed(2)
  ...
  const imintemp = data[0].main.temp_min
  const imaxtemp = data[0].main.temp_max

  // Lowest Temperature of the Day
  lowestToday.innerText = Math.round(Tempconverter(imintemp))
  console.log(data[0].main.temp_min)

  // Highest Temperature of the Day
  highestToday.innerText = Math.round(Tempconverter(imaxtemp))
  console.log(data[0].main.temp_max)
